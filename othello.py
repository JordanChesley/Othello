...
def play():
  ...
  while True:
    ...
    if len(playable_spaces) == 0:
      print(f"{"WHITE" if team else "BLACK"}: Cannot place piece.")
      skipped_turns += 1
      move += 1
      continue
    ...
    while (row, column) not in playable_spaces:
      ...
      if team == BLACK:
        ...
        if (not splinput[0].isdigit()) or (not splinput[1].isdigit()):
          print('Invalid input.')
          continue
        row = int(splinput[0]) - 1; column = int(splinput[1]) - 1
      ...
      if (row, column) not in playable_spaces: print('Cannot place there.')
